...
def upload_running_status():
    ...
    try:
        blocknum = sh(''' curl -X POST -s -H 'Content-Type: application/json' --data '{"jsonrpc":"2.0","method":"ctxc_blockNumber","params":[],"id":83}' 127.0.0.1:''' + str(port))
        enodeInfo = sh(''' curl -X POST -s -H 'Content-Type: application/json' --data '{"jsonrpc":"2.0","method":"admin_nodeInfo","params":[],"id":83}' 127.0.0.1:''' + str(port))
        peersInfo = sh(''' curl -X POST -s -H 'Content-Type: application/json' --data '{"jsonrpc":"2.0","method":"admin_peers","params":[],"id":83}' 127.0.0.1:''' + str(port))
    except BaseException:
        pass
